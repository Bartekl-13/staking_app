  ...
  describe("Staker", function () {
    ...
    describe("   🥩 Stake!", function () {
      it("Balance should go up when you stake()", async function () {
        ...
        expect(txResult.status).to.equal(
          1,
          "Error while awaiting staking confirmation"
        );
      ...
      it("If enough is staked and time has passed, you should be able to complete", async function () {
        ...
        console.log(
          "\t",
          " ⏱  There should be some time left. timeLeft:",
          timeLeft1.toNumber()
        );
        expect(timeLeft1.toNumber()).to.greaterThan(
          0,
        ...
        console.log(
          "\t",
          " ⏱  Time left should be down to 0 now. timeLeft:",
          timeLeft2.toNumber()
        );
        expect(timeLeft2.toNumber()).to.equal(
          0,
        ...
        expect(result).to.equal(
          true,
          "Error while expecting completed to be true."
        );
      });
      ...
      it("Should redeploy Staker, stake, not have enough value, attempt to execute, then withdraw", async function () {
        ...
        expect(txResult.status).to.equal(
          1,
          "Error while awaiting the staking confirmation."
        );
        ...
        expect(result).to.equal(
          false,
          "Error expecting completed to be false."
        );
